...
def launch(ink_dir: str, port: int = 8420) -> None:
    ...
    # 1. Install npm deps if fresh
    import os
    if not os.path.isdir(os.path.join(ink_dir, "node_modules")):
        print("[dim]Installing Node.js dependencies...[/]")
        subprocess.run(["npm", "install"], cwd=ink_dir, capture_output=True, check=True)

    # 2. Install fastapi if missing
    try:
        import fastapi  # noqa: F401
    except ImportError:
        print("[dim]Installing Python API dependencies...[/]")
        subprocess.run([sys.executable, "-m", "pip", "install", "fastapi", "uvicorn"], capture_output=True)
    ...
    for _ in range(15):
        ...
        print("[yellow]Server may still be starting...[/]")
